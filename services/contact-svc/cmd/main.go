...
import (
	...
	"github.com/shulian-paas/im/contact-svc/internal/config"
	"github.com/shulian-paas/im/contact-svc/internal/handler"
	"github.com/shulian-paas/im/contact-svc/internal/middleware"
...
func main() {
	...
	svc := service.NewContactService(mysqlRepo, cache)
	...
	go func() {
		log.Info().Int("port", cfg.Server.Port).Msg("contact-svc starting")
...
func setupRouter(svc *service.ContactService, cfg *config.Config) *gin.Engine {
	r := gin.New()
	r.Use(gin.Recovery())
