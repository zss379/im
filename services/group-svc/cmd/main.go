...
import (
	...
	"github.com/shulian-paas/im/group-svc/internal/config"
	"github.com/shulian-paas/im/group-svc/internal/handler"
	"github.com/shulian-paas/im/group-svc/internal/middleware"
...
func main() {
	...
	svc := service.NewGroupService(mysqlRepo, cache)
	...
	go func() {
		log.Info().Int("port", cfg.Server.Port).Msg("group-svc starting")
...
func setupRouter(svc *service.GroupService, cfg *config.Config) *gin.Engine {
	r := gin.New()
	r.Use(gin.Recovery())
	...
	api := r.Group("/api/v1")
	{
		groupHandler := handler.NewGroupHandler(svc)
