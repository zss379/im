...
import (
	...
	"github.com/shulian-paas/im/auth-svc/internal/config"
	"github.com/shulian-paas/im/auth-svc/internal/handler"
	"github.com/shulian-paas/im/auth-svc/internal/middleware"
...
func main() {
	...
	svc := service.NewAuthService(mysqlRepo, cache, cfg.Server.JWTSecret, cfg.Server.TokenExpiryHours)
	...
	go func() {
		log.Info().Int("port", cfg.Server.Port).Msg("auth-svc starting")
...
func setupRouter(svc *service.AuthService, cfg *config.Config) *gin.Engine {
	r := gin.New()
	r.Use(gin.Recovery())
	...
	r.POST("/api/v1/auth/login", func(c *gin.Context) {
		handler.NewAuthHandler(svc).Login(c)
	})
	r.POST("/api/v1/auth/refresh", func(c *gin.Context) {
		handler.NewAuthHandler(svc).Refresh(c)
	})

	// Auth-required routes
	api := r.Group("/api/v1")
	api.Use(middleware.AuthMiddleware(cfg.Server.JWTSecret))
	{
